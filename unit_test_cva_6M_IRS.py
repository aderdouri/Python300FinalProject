...
class cva_tests(TestCase):    
    def test_01_cva(self):    
        ...
        sigma = 0.0 # fwd rates volatility
        ...
        df[['SpotRate', 'forwardRate']].plot(title='Term structure')
        plt.show()

        print('ExpectedExposure: {0}'.format(ExpectedExposure))

        fig = plt.figure()
        ax = fig.add_subplot(111)
        N = len(ExpectedExposure) ## the data        
        ind = np.arange(N) # the x locations for the groups
        width = 0.35       # the width of the bars
        rects1 = ax.bar(ind, ExpectedExposure, width, color='orange')	
        plt.plot(ExpectedExposure)
        plt.title('IRS Exposure')
        plt.show()


        fig = plt.figure()
        ax = fig.add_subplot(111)
        N = len(CVA_PERIOD) ## the data        
        ind = np.arange(N) # the x locations for the groups
        width = 0.35       # the width of the bars
        rects1 = ax.bar(ind, CVA_PERIOD, width, color='orange')	
        plt.plot(CVA_PERIOD)
        plt.title('CVA PERIOD')
        plt.show()


    def test_02_cva(self):
        return
        df = load_data('cvaDecreasing')
        Notional = df['Notional'][0]
        K = df['FixedRate'][0] # fixed rate IRS
        tau = df['Tau'][0] # daycount factor
        RR = df['RecoveryRate'][0]# Recovery Rate
        ...
        rects1 = ax.bar(ind, ExpectedExposure, width, color='orange')	
        plt.plot(ExpectedExposure)
        plt.title('IRS Exposure Decreasing')
        plt.show()
        ...
        rects1 = ax.bar(ind, CVA_PERIOD, width, color='orange')	
        plt.plot(CVA_PERIOD)
        plt.title('CVA_PERIOD Decreasing')
        plt.show()


    def test_03_cva(self):
        df = load_data('cvaInput')
        Notional = df['Notional'][0]
        K = df['FixedRate'][0] # fixed rate IRS
        tau = df['Tau'][0] # daycount factor
        RR = df['RecoveryRate'][0]# Recovery Rate
        ...
        df[['SpotRate', 'forwardRate']].plot(title='Term structure Decreasing')
        plt.show()


        myplot(ExpectedExposure, 'IncreasingExpectedExposure.png', 'IRS Exposure Increasing')

        myplot(CVA_PERIOD, 'IncreasingCVAPeriod .png', 'CVA_PERIOD Increasing')
